...
class ContractorEditProfileScreen extends ConsumerStatefulWidget {
  const ContractorEditProfileScreen({super.key});

  @override
  ConsumerState<ContractorEditProfileScreen> createState() =>
      _ContractorEditProfileScreenState();
}

class _ContractorEditProfileScreenState extends ConsumerState<ContractorEditProfileScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    profile.whenData((data) {
      if (data != null) {
        ...
        // Initialize selected service dropdown value from profile
        if (selectedService == null && servicesAsync.hasValue) {
          final contractorData = data.data.user.contractor;
          String currentServiceId = '';
          if (contractorData is Map) {
            currentServiceId = contractorData['serviceId'] ?? '';
          }
          if (currentServiceId.isNotEmpty && servicesAsync.value!.isNotEmpty) {
            final found = servicesAsync.value!.cast<ServiceCategory?>().firstWhere(
              (element) => element?.id == currentServiceId,
              orElse: () => null,
            );
            if (found != null) {
              Future.microtask(() {
                ref.read(selectedServiceProvider.notifier).select(found);
              });
            }
          }
        }
